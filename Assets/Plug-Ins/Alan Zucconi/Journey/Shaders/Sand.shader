...
Shader "Alan Zucconi/Journey/Journey Sand" {
	...
	SubShader {
		...
		inline float4 LightingJourney (SurfaceOutputStandard s, fixed3 viewDir, UnityGI gi)
		{
			// Original colour
			...
			// Multiplying by original color applies Unity lights.
			return float4(color * s.Albedo, 1) * pbr;
		}

		void LightingJourney_GI(SurfaceOutputStandard s, UnityGIInput data, inout UnityGI gi)
		{
